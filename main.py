...
MODEL = tf.keras.models.load_model("models/23")
#endpoint = "http://localhost:8501/v1/models/potatoes_model:predict"

CLASS_NAMES = ["all motor vehicle prohibited", "axle road limit","barrier ahead sign" ,"bullock cart and hand cart prohibited sign","cattle traffic sign","compulsory ahead only sign","compulsory cycle track sign","compulsory keep left sign","compulsory keep right sign","speed limit 30"]
...
@app.get("/dd",response_class=HTMLResponse)
async def read_item(request: Request):
    return templates.TemplateResponse("Disease Details.html",{"request":request})

@app.get("/fr",response_class=HTMLResponse)
async def read_item(request: Request):
    return templates.TemplateResponse("Fertilizer Recommendation.html",{"request":request})
...
async def predict(
    ...
    engine.say(predicted_class,250)
    engine.runAndWait()
